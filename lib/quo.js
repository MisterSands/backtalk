...
export function createQuoClient({
  ...
  return {
    ...
    /** Workspace line directory: id, name, number, users on each line. */
    listPhoneNumbers: () => request("GET", "/phone-numbers"),

    /**
     * Calls on one line with one participant — the API requires both filters.
     * Used by the backfill CLI; the webhook path never lists calls.
     */
    listCalls: ({ phoneNumberId, participants, createdAfter, maxResults = 100, pageToken } = {}) =>
      request("GET", "/calls", {
        query: {
          phoneNumberId,
    ...
    /** Conversations, newest first — used by backfill to discover participants. */
    listConversations: ({ createdAfter, maxResults = 100, pageToken } = {}) =>
