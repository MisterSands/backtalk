 ...
 * @param {string} [ctx.lowConfidenceMode="drop"] "drop" = below-threshold items
 *   are removed (legacy behavior); "review" = they survive flagged
 *   `review: true` and are filed with a "[Review]" title prefix instead of
...
export function parseAndValidate(rawText, ctx = {}) {
  const {
    ...
    lowConfidenceMode = "drop",
    nowIso = null, // injectable for tests; defaults to processing time
  } = ctx;
  ...
  const nowParsed = nowIso ? Date.parse(nowIso) : NaN;
  ...
  parsed.commitments.forEach((item, index) => {
    ...
    if (CONFIDENCE_RANK[confidence] < minRank) {
      if (lowConfidenceMode === "review") {
        // Triage instead of discard: keep the item, flag it for human review.
        audit.push({ index, field: "confidence", reason: "low_confidence_review", matched: confidence, dropped: false });
        review = true;
      } else {
    ...
    if (dueIso !== null) {
      ...
      if (!inWindow) {
        ...
        // Valid when spoken, but already past at PROCESSING time (delayed
        // delivery, replay, retry). Quo rejects past due dates outright, so
        // null it — "Spoken due:" in the description preserves the intent.
        audit.push({ index, field: "due_iso", reason: "due_in_past_nulled", matched: dueIso.slice(0, 40), dropped: false });
        dueIso = null;
      }
    }
...
export function buildTaskPayload(commitment, { callId, index, assignedTo = null } = {}) {
  const reviewPrefix = commitment?.review ? "[Review] " : "";
  const payload = {
    title: `${reviewPrefix}${String(commitment?.title ?? "")}`.slice(0, 80),
    description: [
      String(commitment?.detail ?? ""),
      `Quote: "${String(commitment?.verbatim_quote ?? "")}"`,
      `Spoken due: ${commitment?.due_spoken ?? "n/a"}`,
      `Source: backtalk ref:${callId}/${index}`,
    ].join("\n"),
    activityId: callId,
  };
