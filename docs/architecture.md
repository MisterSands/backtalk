...
quo-hangup-hook is a deliberately small machine: one HTTP server, four stages, no database, no queue, no framework. This document explains the pipeline, the event model, the idempotency design, and the reasoning behind the parts that look opinionated.
