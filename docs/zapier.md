...
for (const item of Array.isArray(parsed.commitments) ? parsed.commitments : []) {
  ...
  const payload = {
    ...
    description:
      ...
      '\nSource: quo-hangup-hook ref:' + inputData.call_id + '/' + n,
...
- **Headers:**
  ...
  - `Content-Type` → `application/json`
  - `User-Agent` → `quo-hangup-hook-zapier/0.1`
...
The task lands in Quo linked to the call via `activityId`, with the due date when one was spoken, and a `Source: quo-hangup-hook ref:ACfictional0000000001/1` marker in the description.
